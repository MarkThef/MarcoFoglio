...
#                 # Clear and re-plot
#                 self.ax.clear()
#                 self.ax.fill_between(self.totali_lordi_per_mese.index, self.totali_lordi_per_mese.values, color='#3498db', alpha=0.5)

#                 # Add markers only for non-zero values
#                 non_zero_mask = self.totali_lordi_per_mese > 0
#                 scatter = self.ax.plot(self.totali_lordi_per_mese.index[non_zero_mask], self.totali_lordi_per_mese.values[non_zero_mask], 'o', color='#2980b9', markersize=4)
...
#                     sel.annotation.get_bbox_patch().set(fc="#f39c12", alpha=0.8)
...
#                     self.ax.text(date, -0.03, month_name, ha='center', va='top', fontsize=7.5, fontweight='bold', color='#34495e', rotation=90, transform=self.ax.get_xaxis_transform())

#                 # Distance the year labels downwards
#                 self.ax.xaxis.set_tick_params(which='major', pad=28)

#                 # Set title and labels
#                 self.ax.set_title('Fatturati Mensili', fontsize=16, fontweight='bold', color='#2c3e50')
