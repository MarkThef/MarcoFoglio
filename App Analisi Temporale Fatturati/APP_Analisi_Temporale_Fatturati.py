...
#         self.filtered_df = pd.DataFrame()  # Add filtered_df attribute
#         self.date_col = None
#         self.value_col = None
...
#             # Calculate total revenues per month and year
...
#             self.filtered_df = self.df  # Initialize filtered_df with the full data
...
#         # Set the background color only for the Axes area
...
#                 # Add markers only for non-zero values
...
#                 # Add month labels in Italian above the x-axis
...
#                 # Distance the year labels downwards
...
class MyMainWindow(QMainWindow):
    def __init__(self):
        ...
        self.init_ui()
        self.df = pd.DataFrame()
        self.filtered_df = pd.DataFrame()  # Aggiunge l'attributo filtered_df
        self.date_col = None
        self.value_col = None
    ...
    def show_all_years(self):
        self.year_dropdown.setCurrentIndex(0)  # Resetta la selezione del menu a tendina
        self.filtered_df = self.df
        self.canvas.update_plot(self.df, self.date_col, self.value_col)
    ...
    def show_column_selection_dialog(self, df):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            df[self.date_col] = pd.to_datetime(df[self.date_col], errors='coerce')
            self.df = df.dropna(subset=[self.date_col, self.value_col])
            self.filtered_df = self.df  # Inizializza filtered_df con i dati completi
            self.canvas.load_external_data(self.df, self.date_col, self.value_col)
            self.populate_year_dropdown(self.df)
    ...
    def update_plot(self, df, date_col, value_col):
        if not df.empty and date_col and value_col:
            try:
                ...
                def on_add(sel):
                    sel.annotation.set_text(f'€{sel.target[1]:.2f}')
                    sel.annotation.draggable(True)  # Permette all'annotazione di essere trascinabile
